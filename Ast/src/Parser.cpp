...
// LUAU_NOINLINE is used to limit the stack cost of this function due to std::string object / exception plumbing
    ...
    ParseResult Parser::parse(const char *buffer, size_t bufferSize, AstNameTable &names, Allocator &allocator,
    ...
    Parser::Parser(const char *buffer, size_t bufferSize, AstNameTable &names, Allocator &allocator,
            ...
            : options(options), lexer(buffer, bufferSize, names), allocator(allocator), recursionCounter(0),
              endMismatchSuspect(Location(), Lexeme::Eof), localMap(AstName()) {
        Function top;
        top.vararg = true;
    ...
    bool Parser::blockFollow(const Lexeme &l) {
        return l.type == Lexeme::Eof || l.type == Lexeme::ReservedElse || l.type == Lexeme::ReservedElseif ||
...
// laststat ::= return [explist] | break
    AstStat *Parser::parseStat() {
        ...
        return reportStatError(expr->location, copy({expr}), {},
...
// break
    AstStat *Parser::parseBreak() {
        ...
        if (functionStack.back().loopDepth == 0)
            return reportStatError(start, {}, copy<AstStat *>({allocator.alloc<AstStatBreak>(start)}),
...
// continue
    AstStat *Parser::parseContinue(const Location &start) {
        if (functionStack.back().loopDepth == 0)
            return reportStatError(start, {}, copy<AstStat *>({allocator.alloc<AstStatContinue>(start)}),
...
// funcname ::= Name {`.' Name} [`:' Name]
    AstStat *Parser::parseFunctionStat() {
        ...
        while (lexer.current().type == '.') {
            ...
            expr = allocator.alloc<AstExprIndexName>(Location(start, name.location), expr, name.name, name.location,
                                                     opPosition, '.');
...
// type Name [`<' varlist `>'] `=' typeannotation
    AstStat *Parser::parseTypeAlias(const Location &start, bool exported) {
        ...
        return allocator.alloc<AstStatTypeAlias>(Location(start, type->location), name->name, generics, genericPacks,
                                                 type, exported);
    }
    ...
    AstDeclaredClassProp Parser::parseDeclaredClassMethod() {
        ...
        AstTypeList retTypes = parseOptionalReturnTypeAnnotation().value_or(
        ...
        if (args.size() == 0 || args[0].name.name != "self" || args[0].annotation != nullptr) {
            return AstDeclaredClassProp{fnName.name,
                                        reportTypeAnnotationError(Location(start, end), {}, /*isMissing*/ false,
        ...
        for (size_t i = 1; i < args.size(); ++i) {
            ...
            if (args[i].annotation)
                ...
                vars.push_back(reportTypeAnnotationError(
                        Location(start, end), {}, /*isMissing*/ false,
        ...
        AstType *fnType = allocator.alloc<AstTypeFunction>(
                Location(start, end), generics, genericPacks, AstTypeList{copy(vars), varargAnnotation}, copy(varNames),
                retTypes);
    ...
    AstStat *Parser::parseDeclaration(const Location &start) {
        ...
        if (lexer.current().type == Lexeme::ReservedFunction) {
            ...
            return allocator.alloc<AstStatDeclareFunction>(
                    Location(start, end), globalName.name, generics, genericPacks,
            ...
            return allocator.alloc<AstStatDeclareClass>(Location(classStart, classEnd), className.name, superName,
                                                        copy(props));
    ...
    static bool isExprLValue(AstExpr *expr) {
        return expr->is<AstExprLocal>() || expr->is<AstExprGlobal>() || expr->is<AstExprIndexExpr>() ||
...
// varlist `=' explist
    AstStat *Parser::parseAssignment(AstExpr *initial) {
        if (!isExprLValue(initial))
            initial = reportExprError(initial->location, copy({initial}),
        ...
        while (lexer.current().type == ',') {
            ...
            if (!isExprLValue(expr))
                expr = reportExprError(expr->location, copy({expr}),
                                       "Assigned expression must be a variable or a field");
        ...
        return allocator.alloc<AstStatAssign>(Location(initial->location, values.back()->location), copy(vars),
                                              copy(values));
    }
...
// parlist ::= bindinglist [`,' `...'] | `...'
    std::pair<AstExprFunction *, AstLocal *> Parser::parseFunctionBody(
        ...
        std::optional<Location> argLocation =
                matchParen.type == Lexeme::Type('(') && lexer.current().type == Lexeme::Type(')')
        ...
        return {allocator.alloc<AstExprFunction>(Location(start, end), generics, genericPacks, self, copy(vars), vararg,
                                                 body, functionStack.size(),
...
// bindinglist ::= (binding | `...') [`,' bindinglist]
    std::pair<std::optional<Location>, AstTypePack *>
...
// TypeList ::= TypeAnnotation [`,' TypeList] | ...TypeAnnotation
    AstTypePack *
    Parser::parseTypeList(TempVector<AstType *> &result, TempVector<std::optional<AstArgumentName>> &resultNames) {
        while (true) {
    ...
    std::optional<AstTypeList> Parser::parseOptionalReturnTypeAnnotation() {
        if (options.allowTypeAnnotations &&
            (lexer.current().type == ':' || lexer.current().type == Lexeme::SkinnyArrow)) {
...
// ReturnType ::= TypeAnnotation | `(' TypeList `)'
    std::pair<Location, AstTypeList> Parser::parseReturnTypeAnnotation() {
        ...
        fallbackReturnTypes.push_back(
                parseFunctionTypeAnnotationTail(begin, generics, genericPacks, types, names, varargAnnotation));

        return {Location{location, fallbackReturnTypes[0]->location},
                AstTypeList{copy(fallbackReturnTypes), varargAnnotation}};
    }
            ...
            if (lexer.current().type == '[' &&
                ...
                if (indexer) {
                    ...
                    indexer = parseTableIndexerAnnotation();
                       ...
                       !(lexer.current().type == Lexeme::Name && lexer.lookahead().type == ':')) {
                AstType *type = parseTypeAnnotation();
...
// FunctionTypeAnnotation ::= [`<' varlist `>'] `(' [TypeList] `)' `->` ReturnType
    AstTypeOrPack Parser::parseFunctionTypeAnnotation(bool allowPack) {
        ...
        if (!forceFunctionType && !returnTypeIntroducer && allowPack) {
            ...
            return {{},
                    allocator.alloc<AstTypePackExplicit>(begin.location, AstTypeList{paramTypes, varargAnnotation})};
        }

        AstArray<std::optional<AstArgumentName>> paramNames = copy(names);

        return {parseFunctionTypeAnnotationTail(begin, generics, genericPacks, paramTypes, paramNames,
                                                varargAnnotation), {}};
    }

    AstType *Parser::parseFunctionTypeAnnotationTail(const Lexeme &begin, AstArray<AstGenericType> generics,
                                                     AstArray<AstGenericTypePack> genericPacks,
        ...
        else if (lexer.current().type != Lexeme::SkinnyArrow && generics.size == 0 && genericPacks.size == 0 &&
                 params.size == 0) {
        ...
        return allocator.alloc<AstTypeFunction>(Location(begin.location, endLocation), generics, genericPacks,
...
//   | [`<' varlist `>'] `(' [TypeList] `)' `->` ReturnType
    AstTypeOrPack Parser::parseSimpleTypeAnnotation(bool allowPack) {
        ...
        if (lexer.current().type == Lexeme::ReservedNil) {
            ...
            if (std::optional<AstArray<char>> value = parseCharArray()) {
                ...
                return {reportTypeAnnotationError(begin, {}, /*isMissing*/ false,
            ...
            return {allocator.alloc<AstTypeReference>(Location(begin, end), prefix, name.name, hasParameters,
                                                      parameters), {}};
        } else if (lexer.current().type == '{') {
            ...
            return {reportTypeAnnotationError(location, {}, /*isMissing*/ true, "Expected type, got %s",
    ...
    Parser::checkBinaryConfusables(const BinaryOpPriority binaryPriority[], unsigned int limit) {
        const Lexeme &curr = lexer.current();
        ...
        if (curr.type == '&' && next.type == '&' && curr.location.end == next.location.begin &&
...
// NAME
    AstExpr *Parser::parseNameExpr(const char *context) {
        ...
        if (!name)
            return allocator.alloc<AstExprError>(lexer.current().location, copy<AstExpr *>({}),
        ...
        if (value && *value) {
            ...
            return allocator.alloc<AstExprLocal>(name->location, local,
                                                 local->functionDepth != functionStack.size() - 1);
        }
...
// prefixexp -> NAME | '(' expr ')'
    AstExpr *Parser::parsePrefixExpr() {
        if (lexer.current().type == '(') {
            ...
            if (lexer.current().type != ')') {
                const char *suggestion = (lexer.current().type == '=')
                                         ? "; did you mean to use '{' when defining a table?" : nullptr;
    ...
    AstExpr *Parser::parsePrimaryExpr(bool asStatement) {
        ...
        while (true) {
            if (lexer.current().type == '.') {
                ...
                expr = allocator.alloc<AstExprIndexName>(Location(start, index.location), expr, index.name,
                                                         index.location, opPosition, '.');
            } else if (lexer.current().type == '[') {
                ...
                AstExpr *func = allocator.alloc<AstExprIndexName>(Location(start, index.location), expr, index.name,
                                                                  index.location, opPosition, ':');
            ...
            } else if (lexer.current().type == '{' || lexer.current().type == Lexeme::RawString ||
...
// args ::=  `(' [explist] `)' | tableconstructor | String
    AstExpr *Parser::parseFunctionArgs(AstExpr *func, bool self, const Location &selfLocation) {
        if (lexer.current().type == '(') {
            ...
            return allocator.alloc<AstExprCall>(Location(func->location, end), func, copy(args), self,
            ...
            return allocator.alloc<AstExprCall>(Location(func->location, expr->location), func, copy(&expr, 1), self,
                                                Location(argStart, argEnd));
        } else if (lexer.current().type == Lexeme::RawString || lexer.current().type == Lexeme::QuotedString) {
...
// Name
                ...
                if (lexer.current().type == Lexeme::Dot3 || seenPack) {
                    ...
                    if (withDefaultValues && lexer.current().type == '=') {
                        ...
                        if (shouldParseTypePackAnnotation(lexer)) {
                            ...
                            if (type)
                                report(Location(packBegin.location.begin, lexer.previousLocation().end),
    ...
    AstLocal *Parser::pushLocal(const Binding &binding) {
        ...
        local = allocator.alloc<AstLocal>(
                name.name, name.location, /* shadow= */ local, functionStack.size() - 1, functionStack.back().loopDepth,
                binding.annotation);
...
// cold
    LUAU_NOINLINE void Parser::expectAndConsumeFail(Lexeme::Type type, const char *context) {
        ...
        if (context)
            report(lexer.current().location, "Expected %s when parsing %s, got %s", typeString.c_str(), context,
    ...
    bool Parser::expectMatchAndConsume(char value, const Lexeme &begin, bool searchForMissing) {
        ...
        if (lexer.current().type != type) {
            ...
            if (searchForMissing) {
                ...
                while (currentLine == lexer.current().location.begin.line && lexemeType != type &&
                       matchRecoveryStopOnToken[lexemeType] == 0) {
                    nextLexeme();
...
// cold
    LUAU_NOINLINE void Parser::expectMatchAndConsumeFail(Lexeme::Type type, const Lexeme &begin, const char *extra) {
        ...
        if (lexer.current().location.begin.line == begin.location.begin.line)
            report(lexer.current().location, "Expected %s (to close %s at column %d), got %s%s", typeString.c_str(),
                   begin.toString().c_str(),
                   begin.location.begin.column + 1, lexer.current().toString().c_str(), extra ? extra : "");
        else
            report(lexer.current().location, "Expected %s (to close %s at line %d), got %s%s", typeString.c_str(),
                   begin.toString().c_str(),
    ...
    bool Parser::expectMatchEndAndConsume(Lexeme::Type type, const Lexeme &begin) {
        if (lexer.current().type != type) {
            ...
            if (lexer.current().location.begin.line != begin.location.begin.line &&
                ...
                endMismatchSuspect.location.begin.line <
                begin.location.begin.line) // Only replace the previous suspect with more recent suspects
            {
...
// cold
    LUAU_NOINLINE void Parser::expectMatchEndAndConsumeFail(Lexeme::Type type, const Lexeme &begin) {
        if (endMismatchSuspect.type != Lexeme::Eof &&
            endMismatchSuspect.location.begin.line > begin.location.begin.line) {
            std::string suggestion =
                    format("; did you forget to close %s at line %d?", endMismatchSuspect.toString().c_str(),
                           endMismatchSuspect.location.begin.line + 1);
    ...
    void Parser::incrementRecursionCounter(const char *context) {
        ...
        if (recursionCounter > unsigned(FInt::LuauRecursionLimit)) {
            ParseError::raise(lexer.current().location,
                              "Exceeded allowed recursion depth; simplify your %s to make the code compile", context);
        }
    }
    ...
    LUAU_NOINLINE void Parser::reportNameError(const char *context) {
        if (context)
            report(lexer.current().location, "Expected identifier when parsing %s, got %s", context,
                   lexer.current().toString().c_str());
        else
    ...
    AstStatError *Parser::reportStatError(
            const Location &location, const AstArray<AstExpr *> &expressions, const AstArray<AstStat *> &statements,
            const char *format, ...) {
        va_list args;
        va_start(args, format);
        report(location, format, args);
    ...
    AstExprError *
    Parser::reportExprError(const Location &location, const AstArray<AstExpr *> &expressions, const char *format, ...) {
        va_list args;
        va_start(args, format);
        report(location, format, args);
        va_end(args);
    ...
    AstTypeError *
    Parser::reportTypeAnnotationError(const Location &location, const AstArray<AstType *> &types, bool isMissing,
                                      const char *format, ...) {
        va_list args;
        va_start(args, format);
        report(location, format, args);
