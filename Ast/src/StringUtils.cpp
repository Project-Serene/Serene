...
namespace Luau {
    ...
    size_t editDistance(std::string_view a, std::string_view b) {
        ...
        for (size_t x = 1; x <= a.size(); ++x) {
            ...
            for (size_t y = 1; y <= b.size(); ++y) {
                ...
                distances[getPos(x + 1, y + 1)] = std::min(std::min(insertion, deletion),
    ...
    bool isIdentifier(std::string_view s) {
        return (s.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ01234567890_") ==
                std::string::npos);
    }
