...
namespace Luau {
    ...
    Lexeme::Lexeme(const Location &location, char character)
            : type(static_cast<Type>(static_cast<unsigned char>(character))), location(location), length(0),
              data(nullptr) {
    }
    ...
    Lexeme::Lexeme(const Location &location, Type type, const char *data, size_t size)
        ...
        LUAU_ASSERT(
                type == RawString || type == QuotedString || type == Number || type == Comment || type == BlockComment);
    }
    ...
    static const char *kReserved[] = {"and", "break", "do", "else", "elseif", "end", "false", "for", "function", "if",
                                      "in", "local", "nil", "not", "or",
    ...
    Lexer::Lexer(const char *buffer, size_t bufferSize, AstNameTable &names)
            : buffer(buffer), bufferSize(bufferSize), offset(0), line(0), lineOffset(0),
              lexeme(Location(Position(0, 0), 0), Lexeme::Eof), names(names), skipComments(false), readNames(true) {
    }
...
// Does *not* consume the closing brace.
    ...
    Lexeme Lexer::readQuotedString() {
        ...
        return Lexeme(Location(start, position()), Lexeme::QuotedString, &buffer[startOffset],
