...
void luaV_doarith(lua_State *L, StkId ra, const TValue *rb, const TValue *rc, TMS op) {
    ...
    if ((b = luaV_tonumber(rb, &tempb)) != NULL && (c = luaV_tonumber(rc, &tempc)) != NULL) {
        ...
        if (vb && vc) {
            switch (op) {
                case TM_ADD: setvvalue(ra, vb[0] + vc[0], vb[1] + vc[1], vb[2] + vc[2], vb[3] + vc[3]);
                    return;
                case TM_SUB: setvvalue(ra, vb[0] - vc[0], vb[1] - vc[1], vb[2] - vc[2], vb[3] - vc[3]);
                    return;
                case TM_MUL: setvvalue(ra, vb[0] * vc[0], vb[1] * vc[1], vb[2] * vc[2], vb[3] * vc[3]);
                    return;
                case TM_DIV: setvvalue(ra, vb[0] / vc[0], vb[1] / vc[1], vb[2] / vc[2], vb[3] / vc[3]);
                    return;
                case TM_UNM: setvvalue(ra, -vb[0], -vb[1], -vb[2], -vb[3]);
                    return;
                default:
                    break;
            }
            ...
            if (c) {
                ...
                switch (op) {
                    case TM_MUL: setvvalue(ra, vb[0] * nc, vb[1] * nc, vb[2] * nc, vb[3] * nc);
                        return;
                    case TM_DIV: setvvalue(ra, vb[0] / nc, vb[1] / nc, vb[2] / nc, vb[3] / nc);
                        return;
                    default:
                        break;
                }
            ...
            if (b) {
                ...
                switch (op) {
                    case TM_MUL: setvvalue(ra, nb * vc[0], nb * vc[1], nb * vc[2], nb * vc[3]);
                        return;
                    case TM_DIV: setvvalue(ra, nb / vc[0], nb / vc[1], nb / vc[2], nb / vc[3]);
                        return;
                    default:
                        break;
                }
...
void luaV_dolen(lua_State *L, StkId ra, const TValue *rb) {
    ...
    if (!ttisnumber(res))
        luaG_runerror(L,
                      "'__len' must return a number"); /* note, we can't access rb since stack may have been reallocated */
}
